    ...
    return got(getPath(token, action, resource, tenant), {
      ...
      .then((response) => {
        ...
        if (!checkApplication(user.app_id, user.trusted_apps)) {
          ...
          return reject({ type: 'urn:dx:as:InvalidRole', message: 'User not have the required role in the application' });
      ...
      .catch((error) => {
        if (error instanceof got.HTTPError) {
          return reject({ type: 'urn:dx:as:InvalidAuthenticationToken', message: 'User not authorized in the application' });
        }
