...
function getPolicy(data) {
  ...
  const resourceInfo = [attribute('urn:oasis:names:tc:xacml:1.0:resource:resource-id', data.resource)];
  if (data.tenant_header) {
    resourceInfo.push(attribute('urn:ngsi-ld:resource:tenant', data.tenant_header));
  }
  ...
  const json = {
    Request: {
      AccessSubject: {
        Attribute: [attribute('urn:oasis:names:tc:xacml:2.0:subject:role', data.roles)]
      },
      Action: {
        Attribute: [attribute('urn:oasis:names:tc:xacml:1.0:action:action-id', data.action)]
      },
      Resource: {
