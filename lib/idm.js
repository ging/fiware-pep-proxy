...
var IDM = (function() {
    ...
    var check_token = function(token, callback, callbackError, cache) {
        ...
        proxy.sendData('http', options, undefined, undefined, function (status, resp) {
            ...
            if (status === 401) {
                // Check the difference between current time and last succesful authentication to determine whether we need to retrieve a new token.
                var diffMs = Math.abs(new Date() - authenticate_with_idm_date);
                var diffMinutes = Math.floor((diffMs/1000)/60);
                if (diffMinutes > 50) {
                    // If longer than 50 minutes ago, get a new token
                    ...
                    authenticate (function (token) {
                        ...
                        authenticate_with_idm_date = new Date();
