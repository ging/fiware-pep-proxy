...
const IDM = (function() {
    ...
    const checkToken = function(token, jwt_expiration, action, resource, app_id, authzforce, callback, callbackError, cache) {
        ...
        checkTokenCache(token, jwt_expiration, action, resource, cache, function(cachedUserInfo) {
            if (cachedUserInfo) {
    ...
    const checkTokenCache = function(token, jwt_expiration, action, resource, cache, callback) {

        if (cache[token]) {
            log.info('Token in cache, checking timestamp...');

            const currentTime = (new Date()).getTime();
            const tokenTime = (token.length <= 40 && is_hex(token)) ? jwt_expiration*1000 : cache[token].date.getTime();
