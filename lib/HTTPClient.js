...
// Latency Measures
const fs = require('fs');
fs.appendFileSync('measures/measures-check-token.csv', 'StartCheck,FinishCheck\n');
...
exports.sendData = function(
  ...
  callbackError =
    ...
    function(status, resp) {
      ...
      if (timeStartRedirect) {
        let appResponse = Date.now()
        fs.appendFileSync('measures/measures-send-cb.csv', timeFinishRedirect+',' +appResponse+'\n');
      }
  ...
  let timeFinishRedirect = Date.now()
  if (timeStartRedirect) {
    fs.appendFileSync('measures/measures-check-token.csv', timeStartRedirect+',' +timeFinishRedirect+'\n');  
  }
