...
// COAPS
const index = require('./lib/dtls');

if (config.coaps.enabled) {
  let dtls;
  try {
    dtls = index.createDTLSServer(
      config.coaps.crt_file,
      config.coaps.key_file,
      config.coaps.port,
      'udp4'
    );
  } catch (err) {
